...
class BwaMem_SamToolsView(BioinformaticsTool):
    ...
    def inputs(self) -> List[ToolInput]:
        return [
            ...
            ToolInput("mates", Fastq(optional=True), position=4, shell_quote=False, doc=None),
    ...
    bwa_additional_inputs = [
        ToolInput("threads", Int(optional=True), default=CpuSelector(default=1), prefix="-t", position=2, shell_quote=False,
                  doc="Number of threads. (default = 1)"),
        ToolInput("minimumSeedLength", Int(optional=True), prefix="-k", position=2, shell_quote=False,
        ...
        ToolInput("bandwidth", Int(optional=True), prefix="-w", position=2, shell_quote=False,
                  doc="Essentially, gaps longer than ${bandWidth} will not be found. Note that the maximum gap length "
                      ...
                      " (Default: 100)"),
        ToolInput("offDiagonalXDropoff", Int(optional=True), prefix="-d", position=2, shell_quote=False,
                  doc="(Z-dropoff): Stop extension when the difference between the best and the current extension "
                      ...
                      "(Default: 100)"),
        ToolInput("reseedTrigger", Float(optional=True), prefix="-r", position=2, shell_quote=False,
        ...
        ToolInput("occurenceDiscard", Int(optional=True), prefix="-c", position=2, shell_quote=False,
                  doc="Discard a MEM if it has more than INT occurence in the genome. "
                      "This is an insensitive parameter. (Default: 10000)"),
        ToolInput("performSW", Boolean(optional=True), prefix="-P", position=2, shell_quote=False,
                  doc="In the paired-end mode, perform SW to rescue missing hits only but "
                      "do not try to find hits that fit a proper pair."),
        ToolInput("matchingScore", Int(optional=True), prefix="-A", position=2, shell_quote=False, doc="Matching score. (Default: 1)"),
        ToolInput("mismatchPenalty", Int(optional=True), prefix="-B", position=2, shell_quote=False,
                  doc="Mismatch penalty. The sequence error rate is approximately: {.75 * exp[-log(4) * B/A]}. "
                      "(Default: 4)"),
        ToolInput("openGapPenalty", Int(optional=True), prefix="-O", position=2, shell_quote=False, doc="Gap open penalty. (Default: 6)"),
        ToolInput("gapExtensionPenalty", Int(optional=True), prefix="-E", position=2, shell_quote=False,
                  doc="Gap extension penalty. A gap of length k costs O + k*E "
                      "(i.e. -O is for opening a zero-length gap). (Default: 1)"),
        ToolInput("clippingPenalty", Int(optional=True), prefix="-L", position=2, shell_quote=False,
        ...
        ToolInput("unpairedReadPenalty", Int(optional=True), prefix="-U", position=2, shell_quote=False,
        ...
        ToolInput("appendComments", Boolean(optional=True), prefix="-C", position=2, shell_quote=False,
        ...
        ToolInput("hardClipping", Boolean(optional=True), prefix="-H", position=2, shell_quote=False,
        ...
        ToolInput("markShorterSplits", Boolean(optional=True), prefix="-M", position=2, shell_quote=False,
                  doc="Mark shorter split hits as secondary (for Picard compatibility)."),
        ToolInput("verboseLevel", Int(optional=True), prefix="-v", position=2, shell_quote=False,
    ...
    samtools_additional_args = [
        ...
        ToolInput("skippedReadsOutputFilename", String(optional=True), position=8, shell_quote=False, prefix="-U",
                  doc="output reads not selected by filters to FILE [null]"),
        ToolInput("referenceIndex", File(optional=True), position=8, shell_quote=False, prefix="-t",
                  doc="FILE listing reference names and lengths (see long help) [null]"),
        ToolInput("intervals", Bed(optional=True), position=8, shell_quote=False, prefix="-L",
                  doc="only include reads overlapping this BED FILE [null]"),
        ToolInput("includeReadsInReadGroup", String(optional=True), position=8, shell_quote=False, prefix="-r",
                  doc="only include reads in read group STR [null]"),
        ToolInput("includeReadsInFile", File(optional=True), position=8, shell_quote=False, prefix="-R",
                  doc="only include reads with read group listed in FILE [null]"),
        ToolInput("includeReadsWithQuality", Int(optional=True), position=8, shell_quote=False, prefix="-q",
                  doc="only include reads with mapping quality >= INT [0]"),
        ToolInput("includeReadsInLibrary", String(optional=True), position=8, shell_quote=False, prefix="-l",
                  doc="only include reads in library STR [null]"),
        ToolInput("includeReadsWithCIGAROps", Int(optional=True), position=8, shell_quote=False, prefix="-m",
                  doc="only include reads with number of CIGAR operations consuming query sequence >= INT [0]"),
        ToolInput("includeReadsWithAllFLAGs", Array(Int(), optional=True), position=8, shell_quote=False, prefix="-f",
                  doc="only include reads with all of the FLAGs in INT present [0]"),
        ToolInput("includeReadsWithoutFLAGs", Array(Int(), optional=True), position=8, shell_quote=False, prefix="-F",
                  doc="only include reads with none of the FLAGS in INT present [0]"),
        ToolInput("excludeReadsWithAllFLAGs", Array(Int(), optional=True), position=8, shell_quote=False, prefix="-G",
                  doc="only EXCLUDE reads with all of the FLAGs in INT present [0] "
                      "fraction of templates/read pairs to keep; INT part sets seed)"),
        ToolInput("useMultiRegionIterator", Boolean(optional=True), position=8, shell_quote=False, prefix="-M",
                  doc="use the multi-region iterator (increases the speed, removes "
                      "duplicates and outputs the reads as they are ordered in the file)"),
        ToolInput("readTagToStrip", String(optional=True), position=8, shell_quote=False, prefix="-x",
                  doc="read tag to strip (repeatable) [null]"),
        ToolInput("collapseBackwardCIGAROps", Boolean(optional=True), position=8, shell_quote=False, prefix="-B",
                  doc="collapse the backward CIGAR operation Specify a single input file format "
                      "option in the form of OPTION or OPTION=VALUE"),
        ToolInput("outputFmt", String(optional=True), position=8, shell_quote=False, prefix="--output-fmt",

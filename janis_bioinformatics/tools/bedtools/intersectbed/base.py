...
class BedToolsIntersectBedBase(BedToolsToolBase, ABC):
    ...
    additional_inputs = [
        ...
        ToolInput(
            ...
            Boolean(optional=True),
            prefix="-loj",
            doc="Perform a 'left outer join'. That is, for each feature in A report each overlap with B.  If no overlaps are found, report a NULL feature for B.",
        ),
        ToolInput(
            "writeOriginalAB",
            Boolean(optional=True),
            prefix="-wo",
            doc="Write the original A and B entries plus the number of base pairs of overlap between the two features. - Overlaps restricted by -f and -r. Only A features with overlap are reported.",
        ),
        ToolInput(
            "writeABBase",
            Boolean(optional=True),
            prefix="-wao",
            doc="Write the original A and B entries plus the number of base pairs of overlap between the two features. - Overlapping features restricted by -f and -r. However, A features w/o overlap are also reported with a NULL B feature and overlap = 0.",
        ),
        ToolInput(
            "modeu",
            Boolean(optional=True),
            prefix="-u",
            doc="Write the original A entry _once_ if _any_ overlaps found in B. - In other words, just report the fact >=1 hit was found. - Overlaps restricted by -f and -r.",
        ),
        ToolInput(
            "modec",
            Boolean(optional=True),
            prefix="-c",
            doc="For each entry in A, report the number of overlaps with B. - Reports 0 for A entries that have no overlap with B. - Overlaps restricted by -f, -F, -r, and -s.",
        ),
        ToolInput(
            "modeC",
            Boolean(optional=True),
            prefix="-C",
            doc="-C	For each entry in A, separately report the number of - overlaps with each B file on a distinct line. - Reports 0 for A entries that have no overlap with B. - Overlaps restricted by -f, -F, -r, and -s.",
        ),
        ToolInput(
            "modev",
            Boolean(optional=True),
            prefix="-v",
            doc="Only report those entries in A that have _no overlaps_ with B. - Similar to 'grep -v' (an homage).",
        ),
        # ToolInput("ubam")
        ToolInput(
            "strandedness",

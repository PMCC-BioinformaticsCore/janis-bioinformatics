...
class GatkSetNmAndUqTagsBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "SetNmAndUqTags"

    def friendly_name(self) -> str:
        return "GATK4: SetNmAndUqTags"

    def tool(self) -> str:
        return "Gatk4SetNmAndUqTags"
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T14:59:23.852044"),
            dateUpdated=datetime.fromisoformat("2020-05-18T14:59:23.852045"),
            documentation="b'USAGE: SetNmAndUqTags [arguments]\nDEPRECATED: Use SetNmMdAndUqTags instead.This tool takes in a coordinate-sorted SAM or BAM and calculatesthe NM, MD, and\nUQ tags by comparing with the reference.<br />This may be needed when MergeBamAlignment was run with SORT_ORDER other\nthan 'coordinate' and thuscould not fix these tags then. The input must be coordinate sorted in order to run. If\nspecified,the MD and NM tags can be ignored and only the UQ tag be set.<br /><h4>Usage example:</h4><pre>java -jar\npicard.jar SetNmMdAndUqTags <br />      R=reference_sequence.fasta <br />      I=sorted.bam <br />      O=fixed.bam <br\n/></pre>\nVersion:4.1.3.0\n",
        )

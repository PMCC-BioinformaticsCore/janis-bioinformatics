...
class GatkFilterFuncotationsBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "FilterFuncotations"

    def friendly_name(self) -> str:
        return "GATK4: FilterFuncotations"

    def tool(self) -> str:
        return "Gatk4FilterFuncotations"

    def inputs(self):
        return [
            ToolInput(
                tag="alleleFrequencyDataSource",
                input_type=Boolean(optional=True),
                prefix="--allele-frequency-data-source",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc=" The allele frequency data source (ExAC or gnomAD) that was used to Funcotate the input VCF.  Required. Possible values: {exac, gnomad} "
                ),
            ),
            ToolInput(
                tag="outputFilename",
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-O) Output VCF file to which filtered variants should be written. Required."
                ),
            ),
            ToolInput(
                tag="refVersion",
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="The version of the Human Genome reference which was used to Funcotate the input VCF. Required. Possible values: {b37, hg19, hg38} "
                ),
            ),
            ToolInput(
                tag="variant",

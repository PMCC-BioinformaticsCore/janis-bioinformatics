...
class GatkAnnotatePairOrientationBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "AnnotatePairOrientation"

    def friendly_name(self) -> str:
        return "GATK4: AnnotatePairOrientation"

    def tool(self) -> str:
        return "Gatk4AnnotatePairOrientation"
    ...
    def inputs(self):
        return [
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-O) Output Somatic SNP/Indel VCF file with additional annotations. Required."
                ),
            ),
            ToolInput(
                tag="variant",
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-cutoff)  Cutoff for the min base quality value(s) to count the read.  These are for bases that overlap the variant.  Default value: 7. "
                ),
            ),
            ToolInput(
                tag="quiet",
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T15:03:10.637746"),
            dateUpdated=datetime.fromisoformat("2020-05-18T15:03:10.637747"),
            documentation="**EXPERIMENTAL FEATURE - USE AT YOUR OWN RISK**\nUSAGE: AnnotatePairOrientation [arguments]\nThis adds fields normally emitted by M2 to a VCF.  There should never be a need to run this tool on a VCF that was\nproduced by M2.\nThe output of this tool should be usable with FilterByOrientationBias.\nThe output of this tool only counts reads that fully overlap (and match) the variant or reference sequence (this is\nrelevant for indels).\nIMPORTANT:  This tool does not produce the exact same F1R2/F2R1 as M2, due to the nature of how M2 calls variants (using\nread likelihoods, whereas this tool uses a base quality filter).\nVersion:4.1.3.0\n",
        )

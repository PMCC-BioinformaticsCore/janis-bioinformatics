...
class GatkCollectRawWgsMetricsBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "CollectRawWgsMetrics"

    def friendly_name(self) -> str:
        return "GATK4: CollectRawWgsMetrics"

    def tool(self) -> str:
        return "Gatk4CollectRawWgsMetrics"
    ...
    def inputs(self):
        return [
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-CAP) Treat positions with coverage exceeding this value as if they had coverage at this value (but calculate the difference for PCT_EXC_CAPPED).  Default value: 100000. "
                ),
            ),
            ToolInput(
                tag="create_index",
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc=" At positions with coverage exceeding this value, completely ignore reads that accumulate beyond this value (so that they will not be considered for PCT_EXC_CAPPED).  Used to keep memory consumption in check, but could create bias if set too low  Default value: 200000. "
                ),
            ),
            ToolInput(
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-Q)  Minimum base quality for a base to contribute coverage. N bases will be treated as having a base quality of negative infinity and will therefore be excluded from coverage regardless of the value of this parameter.  Default value: 3. "
                ),
            ),
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-MQ)  Minimum mapping quality for a read to contribute coverage.  Default value: 0. "
                ),
            ),
            ToolInput(
                tag="quiet",

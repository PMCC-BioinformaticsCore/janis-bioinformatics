...
class GatkLeftAlignAndTrimVariantsBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "LeftAlignAndTrimVariants"

    def friendly_name(self) -> str:
        return "GATK4: LeftAlignAndTrimVariants"

    def tool(self) -> str:
        return "Gatk4LeftAlignAndTrimVariants"
    ...
    def inputs(self):
        return [
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-no-trim)  Do not Trim alleles to remove bases common to all of them  Default value: false. Possible values: {true, false} "
                ),
            ),
            ToolInput(
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T15:07:05.637081"),
            dateUpdated=datetime.fromisoformat("2020-05-18T15:07:05.637082"),
            documentation="USAGE: LeftAlignAndTrimVariants [arguments]\nThis tool takes a VCF file, left-aligns the indels and trims common bases from indels,leaving them with a minimum\nrepresentation. The same indel can often be placed at multiple positions and stillrepresent the same haplotype. While\nthe standard convention with VCF is to place an indel at the left-most positionthis isn't always done, so this tool can\nbe used to left-align them. This tool optionally splits multiallelicsites into biallelics and left-aligns individual\nalleles. Optionally, the tool will not trim common bases from indels.\nVersion:4.1.3.0\n",
        )

...
class GatkMarkDuplicatesSparkBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "MarkDuplicatesSpark"

    def friendly_name(self) -> str:
        return "GATK4: MarkDuplicatesSpark"

    def tool(self) -> str:
        return "Gatk4MarkDuplicatesSpark"
    ...
    def inputs(self):
        return [
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-M) Path to write duplication metrics to. Default value: null."
                ),
            ),
            ToolInput(
                tag="numReducers",
            ...
            ToolInput(
                tag="opticalDuplicatePixelDistance",
                input_type=Int(optional=True),
            ...
            ToolInput(
                tag="allowMultipleSortOrdersInInput",
                input_type=Boolean(optional=True),
                prefix="--allow-multiple-sort-orders-in-input",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc=" Allow non-queryname sorted inputs when specifying multiple input bams.  Default value: false. Possible values: {true, false} "
                ),
            ),
            ToolInput(
            ...
            ToolInput(
                tag="treatUnsortedAsQuerygroupOrdered",
                input_type=Boolean(optional=True),
                prefix="--treat-unsorted-as-querygroup-ordered",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc=" Treat unsorted files as query-group orderd files. WARNING: This option disables a basic safety check and may result in unexpected behavior if the file is truly unordered  Default value: false. Possible values: {true, false} "
                ),
            ),
            ToolInput(
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T14:57:34.718407"),
            dateUpdated=datetime.fromisoformat("2020-05-18T14:57:34.718410"),
            documentation="USAGE: MarkDuplicatesSpark [arguments]\nMarks duplicates on spark\nVersion:4.1.3.0\n",
        )

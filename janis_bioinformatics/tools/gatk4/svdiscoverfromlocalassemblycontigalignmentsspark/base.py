...
class GatkSvDiscoverFromLocalAssemblyContigAlignmentsSparkBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "SvDiscoverFromLocalAssemblyContigAlignmentsSpark"

    def friendly_name(self) -> str:
        return "GATK4: SvDiscoverFromLocalAssemblyContigAlignmentsSpark"

    def tool(self) -> str:
        return "Gatk4SvDiscoverFromLocalAssemblyContigAlignmentsSpark"
    ...
    def inputs(self):
        return [
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-O) prefix for output files (including VCF files and if enabled, the signaling assembly contig's alignments); sample name will be appended after the provided argument  Required. "
                ),
            ),
            ToolInput(
                tag="reference",
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="output query-name sorted SAM files for local assembly contigs whose alignment signature could not be used for emitting un-ambiguous calls  Default value: false. Possible values: {true, false} "
                ),
            ),
            ToolInput(
                tag="debugMode",
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T15:03:02.988558"),
            dateUpdated=datetime.fromisoformat("2020-05-18T15:03:02.988559"),
            documentation="**BETA FEATURE - WORK IN PROGRESS**\nUSAGE: SvDiscoverFromLocalAssemblyContigAlignmentsSpark [arguments]\nThis tool is used in development and should not be of interest to most researchers. It is a prototype of structural\nvariant calling, and has been under active developments. For more stable version, please see\nDiscoverVariantsFromContigAlignmentsSAMSpark. This tool takes a file containing the alignments of assembled contigs\n(typically the output file produced by FindBreakpointEvidenceSpark) and searches for reads with split alignments or\nlarge gaps indicating the presence of structural variation breakpoints. Variations' types are determined by analyzing\nthe signatures of the split alignments, and are written to VCF files in the designated output directory.\nVersion:4.1.3.0\n",
        )

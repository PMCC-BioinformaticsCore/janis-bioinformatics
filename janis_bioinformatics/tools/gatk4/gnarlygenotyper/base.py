...
class GatkGnarlyGenotyperBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "GnarlyGenotyper"

    def friendly_name(self) -> str:
        return "GATK4: GnarlyGenotyper"

    def tool(self) -> str:
        return "Gatk4GnarlyGenotyper"
    ...
    def inputs(self):
        return [
            ...
            ToolInput(
                tag="keepAllSites",
                input_type=Boolean(optional=True),
                prefix="--keep-all-sites",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="Retain low quality and non-variant sites, applying appropriate filters Default value: false. Possible values: {true, false} "
                ),
            ),
            ToolInput(
                tag="lenient",
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-merge-input-intervals)  Boolean flag to read in all data in between intervals.  Improves performance reading from GenomicsDB using large lists of +intervals, as in exome sequencing, especially if GVCF data only exists for specified intervals.  Default value: false. Possible values: {true, false} "
                ),
            ),
            ToolInput(
            ...
            ToolInput(
                tag="outputDatabaseName",
                input_type=String(optional=True),
                prefix="--output-database-name",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-output-db)  File to which the sites-only annotation database derived from these input samples should be written  Default value: null. "
                ),
            ),
            ToolInput(
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T15:01:58.100635"),
            dateUpdated=datetime.fromisoformat("2020-05-18T15:01:58.100637"),
            documentation="**BETA FEATURE - WORK IN PROGRESS**\nUSAGE: GnarlyGenotyper [arguments]\nPerform 'quick and dirty' joint genotyping on one or more samples pre-called with HaplotypeCaller\nVersion:4.1.3.0\n",
        )

...
class GatkAnnotateVcfWithExpectedAlleleFractionBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "AnnotateVcfWithExpectedAlleleFraction"

    def friendly_name(self) -> str:
        return "GATK4: AnnotateVcfWithExpectedAlleleFraction"

    def tool(self) -> str:
        return "Gatk4AnnotateVcfWithExpectedAlleleFraction"
        ...
        return [
            ToolInput(
                ...
                doc=InputDocumentation(
                    doc="(-mixing-fractions)  The input mixing fractions table  Required. "
                ),
            ),
            ToolInput(
                tag="outputFilename",
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-O) The output annotated VCF file Required."
                ),
            ),
            ToolInput(
                tag="variant",
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T15:03:25.487467"),
            dateUpdated=datetime.fromisoformat("2020-05-18T15:03:25.487469"),
            documentation="USAGE: AnnotateVcfWithExpectedAlleleFraction [arguments]\nAnnotate a multi-sample vcf with expected allele fractions in pooled sequencing given mixing fractions of the different\nsamples in the pool via the formula Expected allele fraction = SUM_samples {mixing_fraction(sample) * [0 if hom ref, 0.5\nis het, 1.0 if hom var]}\nVersion:4.1.3.0\n",
        )

...
class GatkCollectVariantCallingMetricsBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "CollectVariantCallingMetrics"

    def friendly_name(self) -> str:
        return "GATK4: CollectVariantCallingMetrics"

    def tool(self) -> str:
        return "Gatk4CollectVariantCallingMetrics"
    ...
    def inputs(self):
        return [
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-I) Input vcf file for analysis Required."
                ),
            ),
            ToolInput(
                tag="outputFilename",
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-O) Path (except for the file extension) of output metrics files to write. Required."
                ),
            ),
            ToolInput(
                tag="arguments_file",
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="Set to true if running on a single-sample gvcf. Default value: false. Possible values: {true, false} "
                ),
            ),
            ToolInput(
                tag="help",
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(speeds, will, -SD)  here.  Default value: null. "
                ),
            ),
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-TI) Target intervals to restrict analysis to. Default value: null."
                ),
            ),
            ToolInput(
                tag="thread_count",
                ...
                prefix="--THREAD_COUNT",
                separate_value_from_prefix=True,
                doc=InputDocumentation(doc="Undocumented option Default value: 1."),
            ),
            ToolInput(
                tag="tmp_dir",
                input_type=File(optional=True),
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T14:11:06.809335"),
            dateUpdated=datetime.fromisoformat("2020-05-18T14:11:06.809336"),
            documentation="b'USAGE: CollectVariantCallingMetrics [arguments]\nCollects per-sample and aggregate (spanning all samples) metrics from the provided VCF file.\nVersion:4.1.3.0\n",
        )

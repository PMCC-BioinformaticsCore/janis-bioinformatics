...
class GatkParallelCopyGCSDirectoryIntoHDFSSparkBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "ParallelCopyGCSDirectoryIntoHDFSSpark"

    def friendly_name(self) -> str:
        return "GATK4: ParallelCopyGCSDirectoryIntoHDFSSpark"

    def tool(self) -> str:
        return "Gatk4ParallelCopyGCSDirectoryIntoHDFSSpark"
    ...
    def inputs(self):
        return [
            ...
            ToolInput(
                tag="outputHdfsDirectory",
                input_type=Boolean(optional=True),
                prefix="--output-hdfs-directory",
                separate_value_from_prefix=True,
                doc=InputDocumentation(doc=" Required. "),
            ),
            ToolInput(
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="optional wildcard glob to subset files in the input directory to copy Default value: *."
                ),
            ),
            ToolInput(
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T14:55:09.313543"),
            dateUpdated=datetime.fromisoformat("2020-05-18T14:55:09.313544"),
            documentation="**BETA FEATURE - WORK IN PROGRESS**\nUSAGE: ParallelCopyGCSDirectoryIntoHDFSSpark [arguments]\nThis tool uses a Spark cluster to do a parallel copy of either a single file or a directory from Google Cloud Storage\n(GCS) into the HDFS file system used by Spark to support Resilient Distributed Datasets (RDDs). Files are divided into\nchunks of size equal to the HDFS block size (with the exception of the final chunk) and each Spark task is responsible\nfor copying one chunk. To copy all of the files in a GCS directory, provide the GCS directory path, including the\ntrailing slash. Directory copies are non-recursive so subdirectories will be skipped. Within directories each file is\ndivided into chunks independently (so this will be inefficient if you have lots of files smaller than the block size).\nAfter all chunks are copied, the HDFS concat method is used to stitch together chunks into single files without\nre-copying them. This functionality is used by the structural variation workflow to copy reference data when a Spark\ncluster is created, and may also be used to copy sample data to a Spark cluster.\nVersion:4.1.3.0\n",
        )

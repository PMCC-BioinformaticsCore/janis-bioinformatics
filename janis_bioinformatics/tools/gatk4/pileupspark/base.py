...
class GatkPileupSparkBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "PileupSpark"

    def friendly_name(self) -> str:
        return "GATK4: PileupSpark"

    def tool(self) -> str:
        return "Gatk4PileupSpark"
    ...
    def inputs(self):
        return [
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-O) The output directory to which the scattered output will be written. Required."
                ),
            ),
            ToolInput(
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-DF)  Read filters to be disabled before analysis  This argument may be specified 0 or more times. Default value: null. Possible Values: {MappedReadFilter, NotDuplicateReadFilter, NotSecondaryAlignmentReadFilter, PassesVendorQualityCheckReadFilter, WellformedReadFilter}"
                ),
            ),
            ToolInput(
            ...
            ToolInput(
                tag="maxDepthPerSample",
                input_type=Int(optional=True),
                prefix="--max-depth-per-sample",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-max-depth-per-sample)  Maximum number of reads to retain per sample per locus. Reads above this threshold will be downsampled. Set to 0 to disable.  Default value: 0. "
                ),
            ),
            ToolInput(
                tag="metadata",
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-metadata)  Features file(s) containing metadata. The overlapping sites will be annotated with the corresponding source Feature identifier.  This argument may be specified 0 or more times. Default value: null. "
                ),
            ),
            ToolInput(
                tag="numReducers",
            ...
            ToolInput(
                tag="outputInsertLength",
                input_type=Boolean(optional=True),
                prefix="--output-insert-length",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-output-insert-length)  If enabled, inserts lengths will be added to the output pileup.  Default value: false. Possible values: {true, false} "
                ),
            ),
            ToolInput(
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-read-shard-size)  Maximum size of each read shard, in bases.  Default value: 10000. "
                ),
            ),
            ToolInput(
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-verbose)  Add extra informative columns to the pileup output. The verbose output contains the number of spanning deletions, and for each read in the pileup it has the read name, offset in the base string, read length, and read mapping quality.  These per read items are delimited with an '@' character.  Default value: false. Possible values: {true, false} "
                ),
            ),
            ToolInput(
                tag="shuffle",
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-shuffle) whether to use the shuffle implementation or overlaps partitioning (the default) Default value: false. Possible values: {true, false} "
                ),
            ),
            ToolInput(
                tag="sparkMaster",
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T14:07:45.999654"),
            dateUpdated=datetime.fromisoformat("2020-05-18T14:07:45.999654"),
            documentation="**BETA FEATURE - WORK IN PROGRESS**\nUSAGE: PileupSpark [arguments]\nPrints read alignments in samtools pileup format. The tool leverages the Spark framework for faster operationThe output\ncomprises one line per genomic position, listing the chromosome name, coordinate, reference base, read bases, and read\nqualities. In addition to these default fields, additional information can be added to the output as extra columns.\nVersion:4.1.3.0\n",
        )

...
class GatkSamToFastqBase(Gatk4ToolBase, ABC):
    @classmethod
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T14:59:10.811568"),
            dateUpdated=datetime.fromisoformat("2020-05-18T14:59:10.811569"),
            documentation="b'USAGE: SamToFastq [arguments]\nConverts a SAM or BAM file to FASTQ. Extracts read sequences and qualities from the input SAM/BAM file and writes them\nintothe output file in Sanger FASTQ format.See <a href='http://maq.sourceforge.net/fastq.shtml'>MAQ FASTQ\nspecification</a> for details.This tool can be used by way of a pipe to run BWA MEM on unmapped BAM (uBAM) files\nefficiently.</p><p>In the RC mode (default is True), if the read is aligned and the alignment is to the reverse strand\non the genome,the read's sequence from input sam file will be reverse-complemented prior to writing it to FASTQ in order\nrestore correctlythe original read sequence as it was generated by the sequencer.</p><br /><h4>Usage\nexample:</h4><pre>java -jar picard.jar SamToFastq <br />     I=input.bam<br />     FASTQ=output.fastq</pre><hr />\nVersion:4.1.3.0\n",
        )

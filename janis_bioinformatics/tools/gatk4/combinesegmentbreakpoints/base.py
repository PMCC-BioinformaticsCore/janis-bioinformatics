...
class GatkCombineSegmentBreakpointsBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "CombineSegmentBreakpoints"

    def friendly_name(self) -> str:
        return "GATK4: CombineSegmentBreakpoints"

    def tool(self) -> str:
        return "Gatk4CombineSegmentBreakpoints"

    def inputs(self):
        return [
            ToolInput(
                tag="columnsOfInterest",
                input_type=String(optional=True),
                prefix="--columns-of-interest",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="List of columns in either segment file that should be reported in the output file. If the column header exists in both, it will have the appropriate label appended as a suffix.  This argument must be specified at least once. Required. "
                ),
            ),
            ToolInput(
                tag="outputFilename",
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-O) Output TSV file with combined segment breakpoints Required."
                ),
            ),
            ToolInput(
                tag="segments",
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="Input segment files -- must be specified twice, but order does not matter. This argument must be specified at least once. Required. "
                ),
            ),
            ToolInput(
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="Input segment file labels -- these will appear as suffixes in case of collisions. The specification order must correspond to the input segment files.  This argument may be specified 0 or more times. Default value: [1, 2]. "
                ),
            ),
            ToolInput(
                tag="lenient",
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T14:05:06.683042"),
            dateUpdated=datetime.fromisoformat("2020-05-18T14:05:06.683043"),
            documentation="**EXPERIMENTAL FEATURE - USE AT YOUR OWN RISK**\nUSAGE: CombineSegmentBreakpoints [arguments]\nCombine the breakpoints of two segment files while preserving annotations.\nThis tool will load all segments into RAM.\nComments lines start with '#' or the data can be prepended by a SAM File Header (not both).\nOutput file comments will be the first segment file comments concatenated with the second file comments.\nSAMFileHeaders in the input seg files are supported and will be merged in the output.\nOutput seg file will have a SAMFileHeader, even if the inputs did not.  If neither input file has a SAMFileHeader, then\na reference must be specified.\nVersion:4.1.3.0\n",
        )

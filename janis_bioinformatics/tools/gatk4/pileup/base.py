...
class GatkPileupBase(Gatk4ToolBase, ABC):
    @classmethod
    ...
    def inputs(self):
        return [
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-O) An output file created to be created by the walker. Will overwrite the contents if the file exists.  Required. "
                ),
            ),
            ToolInput(
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T14:07:39.420928"),
            dateUpdated=datetime.fromisoformat("2020-05-18T14:07:39.420929"),
            documentation="USAGE: Pileup [arguments]\nPrints read alignments in samtools pileup format. The output comprises one line per genomic position, listing the\nchromosome name, coordinate, reference base, read bases, and read qualities. In addition to these default fields,\nadditional information can be added to the output as extra columns.\nVersion:4.1.3.0\n",
        )

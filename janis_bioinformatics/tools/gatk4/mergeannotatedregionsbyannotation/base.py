...
class GatkMergeAnnotatedRegionsByAnnotationBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "MergeAnnotatedRegionsByAnnotation"

    def friendly_name(self) -> str:
        return "GATK4: MergeAnnotatedRegionsByAnnotation"

    def tool(self) -> str:
        return "Gatk4MergeAnnotatedRegionsByAnnotation"

    def inputs(self):
        return [
            ToolInput(
                tag="annotationsToMatch",
            ...
            ToolInput(
                ...
                prefix="--output",
                separate_value_from_prefix=True,
                doc=InputDocumentation(doc="(-O) Output TSV file Required."),
            ),
            ToolInput(
                tag="segments",
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="Input segment file or annotated segment file. Required."
                ),
            ),
            ToolInput(
            ...
            ToolInput(
                tag="maxMergeDistance",
                input_type=Int(optional=True),
                prefix="--max-merge-distance",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="Maximum distance to merge in bases Default value: 1000000."
                ),
            ),
            ToolInput(
                tag="outputContigColumn",
                input_type=Boolean(optional=True),
                prefix="--output-contig-column",
                separate_value_from_prefix=True,
                doc=InputDocumentation(doc="Default value: CONTIG."),
            ),
            ToolInput(
                tag="outputEndColumn",
                input_type=String(optional=True),
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="Output end column name. Default value: END."
                ),
            ),
            ToolInput(
                tag="outputStartColumn",
                input_type=String(optional=True),
                prefix="--output-start-column",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="Output start position column name. Default value: START."
                ),
            ),
            ToolInput(
                tag="quiet",
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T14:05:51.632681"),
            dateUpdated=datetime.fromisoformat("2020-05-18T14:05:51.632683"),
            documentation="**EXPERIMENTAL FEATURE - USE AT YOUR OWN RISK**\nUSAGE: MergeAnnotatedRegionsByAnnotation [arguments]\nThis simple tool will merge genomic regions (specified in a tsv) when given annotations (columns) contain exact values\nin neighboring segments and the segments are within a specified maximum genomic distance.\nVersion:4.1.3.0\n",
        )

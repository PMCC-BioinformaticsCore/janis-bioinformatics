...
class GatkStructuralVariationDiscoveryPipelineSparkBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "StructuralVariationDiscoveryPipelineSpark"

    def friendly_name(self) -> str:
        return "GATK4: StructuralVariationDiscoveryPipelineSpark"

    def tool(self) -> str:
        return "Gatk4StructuralVariationDiscoveryPipelineSpark"
    ...
    def inputs(self):
        return [
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(doc="sam file for aligned contigs Required."),
            ),
            ToolInput(
                tag="inp",
                input_type=String(optional=True),
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-O) directory for VCF output, including those from experimental interpretation tool if so requested, will be created if not present; sample name will be appended after the provided argument  Required. "
                ),
            ),
            ToolInput(
                tag="reference",
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="flag to signal that user wants to run experimental interpretation tool as well Default value: false. Possible values: {true, false} "
                ),
            ),
            ToolInput(
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T15:02:54.945296"),
            dateUpdated=datetime.fromisoformat("2020-05-18T15:02:54.945297"),
            documentation="**BETA FEATURE - WORK IN PROGRESS**\nUSAGE: StructuralVariationDiscoveryPipelineSpark [arguments]\nThis tool packages the algorithms described in FindBreakpointEvidenceSpark and\nDiscoverVariantsFromContigAlignmentsSAMSpark as an integrated workflow.  Please consult the descriptions of those tools\nfor more details about the algorithms employed.  In brief, input reads are examined for evidence of structural variation\nin a genomic region, regions so identified are locally assembled, and the local assemblies are called for structural\nvariation.\nVersion:4.1.3.0\n",
        )

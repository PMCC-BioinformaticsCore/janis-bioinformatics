...
class GatkASEReadCounterBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "ASEReadCounter"

    def friendly_name(self) -> str:
        return "GATK4: ASEReadCounter"

    def tool(self) -> str:
        return "Gatk4ASEReadCounter"
    ...
    def inputs(self):
        return [
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-V) One or more VCF files This argument must be specified at least once. Required."
                ),
            ),
            ToolInput(
            ...
            ToolInput(
                tag="countOverlapReadsHandling",
                input_type=Boolean(optional=True),
                prefix="--count-overlap-reads-handling",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-overlap)  Handling of overlapping reads from the same fragment  Default value: COUNT_FRAGMENTS_REQUIRE_SAME_BASE. Possible values: {COUNT_READS, COUNT_FRAGMENTS, COUNT_FRAGMENTS_REQUIRE_SAME_BASE} "
                ),
            ),
            ToolInput(
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-DF)  Read filters to be disabled before analysis  This argument may be specified 0 or more times. Default value: null. Possible Values: {HasReadGroupReadFilter, MappedReadFilter, MatchingBasesAndQualsReadFilter, NotDuplicateReadFilter, NotSecondaryAlignmentReadFilter, SeqIsStoredReadFilter, ValidAlignmentEndReadFilter, ValidAlignmentStartReadFilter}"
                ),
            ),
            ToolInput(
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-mbq) Minimum base quality Default value: 0."
                ),
            ),
            ToolInput(
                tag="minDepthOfNonFilteredBase",
                input_type=Int(optional=True),
                prefix="--min-depth-of-non-filtered-base",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-min-depth)  Minimum number of bases that pass filters  Default value: -1. "
                ),
            ),
            ToolInput(
                tag="minMappingQuality",
                input_type=Int(optional=True),
                prefix="--min-mapping-quality",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-mmq)  Minimum read mapping quality  Default value: 0. "
                ),
            ),
            ToolInput(
                tag="outputFilename",
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-O) Output file (if not provided, defaults to STDOUT) Default value: null."
                ),
            ),
            ToolInput(
                tag="outputFormat",
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="Default value: RTABLE. Possible values: {TABLE, RTABLE, CSV}"
                ),
            ),
            ToolInput(
                tag="quiet",
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T14:06:30.327723"),
            dateUpdated=datetime.fromisoformat("2020-05-18T14:06:30.327727"),
            documentation="USAGE: ASEReadCounter [arguments]\nCounts filtered reads at het sites for allele specific expression estimate\nVersion:4.1.3.0\n",
        )

...
class GatkDiscoverVariantsFromContigAlignmentsSAMSparkBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "DiscoverVariantsFromContigAlignmentsSAMSpark"

    def friendly_name(self) -> str:
        return "GATK4: DiscoverVariantsFromContigAlignmentsSAMSpark"

    def tool(self) -> str:
        return "Gatk4DiscoverVariantsFromContigAlignmentsSAMSpark"
    ...
    def inputs(self):
        return [
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-O) prefix for discovery (non-genotyped) VCF; sample name will be appended after the provided argument, followed by '_inv_del_ins.vcf'  Required. "
                ),
            ),
            ToolInput(
                tag="reference",
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T15:02:31.891812"),
            dateUpdated=datetime.fromisoformat("2020-05-18T15:02:31.891813"),
            documentation="**BETA FEATURE - WORK IN PROGRESS**\nUSAGE: DiscoverVariantsFromContigAlignmentsSAMSpark [arguments]\nThis tool is used in development and should not be of interest to most researchers.  It packages structural variant\ncalling as a separate tool, independent of the generation of local assemblies. Most researchers will run\nStructuralVariationDiscoveryPipelineSpark, which both generates local assemblies of interesting genomic regions, and\nthen calls structural variants from these assemblies. This tool takes a SAM/BAM/CRAM containing the alignments of\nassembled contigs from local assemblies and searches it for split alignments indicating the presence of structural\nvariations. To do so the tool parses primary and supplementary alignments; secondary alignments are ignored. To be\nconsidered valid evidence of an SV, two alignments from the same contig must have mapping quality 60, and both\nalignments must have length greater than or equal to min-alignment-length. Imprecise variants with approximate locations\nare also called.\nThe input file is typically the output file produced by FindBreakpointEvidenceSpark.\nVersion:4.1.3.0\n",
        )

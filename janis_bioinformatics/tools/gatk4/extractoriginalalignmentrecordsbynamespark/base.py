...
class GatkExtractOriginalAlignmentRecordsByNameSparkBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "ExtractOriginalAlignmentRecordsByNameSpark"

    def friendly_name(self) -> str:
        return "GATK4: ExtractOriginalAlignmentRecordsByNameSpark"

    def tool(self) -> str:
        return "Gatk4ExtractOriginalAlignmentRecordsByNameSpark"
    ...
    def inputs(self):
        return [
            ...
            ToolInput(
                ...
                prefix="--output",
                separate_value_from_prefix=True,
                doc=InputDocumentation(doc="(-O) file to write reads to Required."),
            ),
            ToolInput(
                tag="readNameFile",
                input_type=String(optional=True),
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-f) file containing list of read names Required."
                ),
            ),
            ToolInput(
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-v) invert the list, i.e. filter out reads whose name appear in the given file Default value: false. Possible values: {true, false} "
                ),
            ),
            ToolInput(
                tag="numReducers",
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T14:56:54.244530"),
            dateUpdated=datetime.fromisoformat("2020-05-18T14:56:54.244531"),
            documentation="**BETA FEATURE - WORK IN PROGRESS**\nUSAGE: ExtractOriginalAlignmentRecordsByNameSpark [arguments]\nReads a file of read (i.e., template) names, and searches a SAM/BAM/CRAM to find names that match. The matching reads\nare copied to an output file. Unlike FilterSamReads (Picard), this tool can take input reads in any order (e.g.,\nunsorted or coordinate-sorted).\nVersion:4.1.3.0\n",
        )

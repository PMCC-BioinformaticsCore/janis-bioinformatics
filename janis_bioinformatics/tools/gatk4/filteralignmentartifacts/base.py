...
class GatkFilterAlignmentArtifactsBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "FilterAlignmentArtifacts"

    def friendly_name(self) -> str:
        return "GATK4: FilterAlignmentArtifacts"

    def tool(self) -> str:
        return "Gatk4FilterAlignmentArtifacts"
    ...
    def inputs(self):
        return [
            ToolInput(
                ...
                doc=InputDocumentation(doc="(-index)  BWA-mem index image  Required. "),
            ),
            ToolInput(
                tag="inp",
                input_type=String(optional=True),
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-O) The output filtered VCF file Required."
                ),
            ),
            ToolInput(
                tag="variant",
            ...
            ToolInput(
                tag="dontSkipFilteredVariants",
                input_type=Boolean(optional=True),
                prefix="--dont-skip-filtered-variants",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc=" Try to realign all variants, even ones that have already been filtered.  Default value: false. Possible values: {true, false} "
                ),
            ),
            ToolInput(
                tag="dontUseMates",
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="Realign individual reads without using their mates Default value: false. Possible values: {true, false} "
                ),
            ),
            ToolInput(
                tag="dropRatio",
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-drop-ratio)  Fraction of best MEM extension score below which other extensions are dropped  Default value: 0.2. "
                ),
            ),
            ToolInput(
            ...
            ToolInput(
                tag="indelStartTolerance",
                input_type=Int(optional=True),
                prefix="--indel-start-tolerance",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc=" Max distance between indel start of aligned read in the bam and the variant in the vcf  Default value: 5. "
                ),
            ),
            ToolInput(
            ...
            ToolInput(
                tag="maxFailedRealignments",
                input_type=Int(optional=True),
                prefix="--max-failed-realignments",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc=" Maximum number of failed read realignments before a variant is rejected.  Default value: 3. "
                ),
            ),
            ToolInput(
                tag="maxReasonableFragmentLength",
                input_type=Int(optional=True),
                prefix="--max-reasonable-fragment-length",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc=" Maximum fragment length to be considered a reasonable pair alignment  Default value: 100000. "
                ),
            ),
            ToolInput(
                tag="minAlignerScoreDifference",
                input_type=Int(optional=True),
                prefix="--min-aligner-score-difference",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc=" Minimum difference between best and second-best alignment for a read to be considered well-mapped  Default value: 20. "
                ),
            ),
            ToolInput(
                tag="minMismatchRatio",
                input_type=Double(optional=True),
                prefix="--min-mismatch-ratio",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="Minimum ratio between the number of mismatches in the second best and best alignments Default value: 2.5. "
                ),
            ),
            ToolInput(
                tag="minimumSeedLength",
                input_type=Int(optional=True),
                prefix="--minimum-seed-length",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-min-seed-length)  Minimum number of matching bases to seed a MEM  Default value: 14. "
                ),
            ),
            ToolInput(
                tag="numRegularContigs",
                input_type=Boolean(optional=True),
                prefix="--num-regular-contigs",
                separate_value_from_prefix=True,
                doc=InputDocumentation(doc="Default value: 25."),
            ),
            ToolInput(
                tag="quiet",
                input_type=Boolean(optional=True),
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-split-factor)  MEMs longer than the minimum seed length times this factor are split and re-seeded.  Default value: 0.5. "
                ),
            ),
            ToolInput(
            ...
            ToolInput(
                tag="sufficientGoodRealignments",
                input_type=Int(optional=True),
                prefix="--sufficient-good-realignments",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc=" Sufficient number of good read realignments to accept a variant.  Default value: 2. "
                ),
            ),
            ToolInput(
                tag="tmpDir",
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T15:06:14.649105"),
            dateUpdated=datetime.fromisoformat("2020-05-18T15:06:14.649106"),
            documentation="**EXPERIMENTAL FEATURE - USE AT YOUR OWN RISK**\nUSAGE: FilterAlignmentArtifacts [arguments]\nFilter alignment artifacts from a vcf callset.\nVersion:4.1.3.0\n",
        )

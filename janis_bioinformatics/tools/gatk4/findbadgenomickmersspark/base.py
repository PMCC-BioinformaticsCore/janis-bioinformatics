...
class GatkFindBadGenomicKmersSparkBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "FindBadGenomicKmersSpark"

    def friendly_name(self) -> str:
        return "GATK4: FindBadGenomicKmersSpark"

    def tool(self) -> str:
        return "Gatk4FindBadGenomicKmersSpark"
        ...
        return [
            ToolInput(
                ...
                doc=InputDocumentation(
                    doc="(-O) file for ubiquitous kmer output Required."
                ),
            ),
            ToolInput(
                tag="reference",
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="additional high copy kmers (mitochondrion, e.g.) fasta file name Default value: null."
                ),
            ),
            ToolInput(
                tag="inp",
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T15:01:10.389467"),
            dateUpdated=datetime.fromisoformat("2020-05-18T15:01:10.389470"),
            documentation="**BETA FEATURE - WORK IN PROGRESS**\nUSAGE: FindBadGenomicKmersSpark [arguments]\nSearch the reference for kmers (fixed-length substrings) that occur more than a specified number of times, and list them\nto an output file.  The resulting output file is appropriate for use as the --kmers-to-ignore input file by the\nStructuralVariationDiscoveryPipelineSpark tool, which will ignore these kmers when trying to produce candidate reads for\nlocal assemblies.\nVersion:4.1.3.0\n",
        )

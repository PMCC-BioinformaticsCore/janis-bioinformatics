...
class GatkValidateVariantsBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "ValidateVariants"

    def friendly_name(self) -> str:
        return "GATK4: ValidateVariants"

    def tool(self) -> str:
        return "Gatk4ValidateVariants"
    ...
    def inputs(self):
        return [
            ...
            ToolInput(
                tag="doNotValidateFilteredRecords",
                input_type=Boolean(optional=True),
                prefix="--do-not-validate-filtered-records",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-do-not-validate-filtered-records)  skip validation on filtered records  Default value: false. Possible values: {true, false}  Cannot be used in conjuction with argument(s) VALIDATE_GVCF (gvcf)"
                ),
            ),
            ToolInput(
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-gvcf) Default value: false. Possible values: {true, false} Cannot be used in conjuction with argument(s) DO_NOT_VALIDATE_FILTERED (do-not-validate-filtered-records)"
                ),
            ),
            ToolInput(
                tag="validationTypeToExclude",
                input_type=Boolean(optional=True),
                prefix="--validation-type-to-exclude",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-Xtype)  which validation type to exclude from a full strict validation  This argument may be specified 0 or more times. Default value: null. Possible values: {ALL, REF, IDS, ALLELES, CHR_COUNTS} "
                ),
            ),
            ToolInput(
                tag="verbosity",
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-warn-on-errors)  just emit warnings on errors instead of terminating the run at the first instance  Default value: false. Possible values: {true, false} "
                ),
            ),
            ToolInput(
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T15:05:16.389448"),
            dateUpdated=datetime.fromisoformat("2020-05-18T15:05:16.389449"),
            documentation="USAGE: ValidateVariants [arguments]\nValidates a VCF file with an extra strict set of criteria.\nVersion:4.1.3.0\n",
        )

...
class GatkCollectMultipleMetricsBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "CollectMultipleMetrics"

    def friendly_name(self) -> str:
        return "GATK4: CollectMultipleMetrics"

    def tool(self) -> str:
        return "Gatk4CollectMultipleMetrics"
    ...
    def inputs(self):
        return [
            ...
            ToolInput(
                ...
                prefix="--OUTPUT",
                separate_value_from_prefix=True,
                doc=InputDocumentation(doc="(-O) Base name of output files. Required."),
            ),
            ToolInput(
                tag="arguments_file",
                input_type=File(optional=True),
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="extra arguments to the various tools can be specified using the following format:<PROGRAM>::<ARGUMENT_AND_VALUE> where <PROGRAM> is one of the programs specified in PROGRAM, and <ARGUMENT_AND_VALUE> are the argument and value that you'd like to specify as you would on the command line. For example, to change the HISTOGRAM_WIDTH in CollectInsertSizeMetrics to 200, use: 'EXTRA_ARGUMENT=CollectInsertSizeMetrics::HISTOGRAM_WIDTH=200' or, in the new parser:--EXTRA_ARGUMENT 'CollectInsertSizeMetrics::--HISTOGRAM_WIDTH 200' (Quotes are required to avoid the shell from separating this into two arguments.) Note that the following arguments cannot be modified on a per-program level: INPUT, REFERENCE_SEQUENCE, ASSUME_SORTED, and STOP_AFTER. Providing them in an EXTRA_ARGUMENT will _not_ result in an error, but they will be silently ignored.   This argument may be specified 0 or more times. Default value: null. "
                ),
            ),
            ToolInput(
                tag="file_extension",
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-EXT) Append the given file extension to all metric file names (ex. OUTPUT.insert_size_metrics.EXT). None if null  Default value: null. "
                ),
            ),
            ToolInput(
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="If a read maps to a sequence specified with this option, all the bases in the read are counted as ignored bases.  This argument may be specified 0 or more times. Default value: null. "
                ),
            ),
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-UNPAIRED)  Include unpaired reads in CollectSequencingArtifactMetrics. If set to true then all paired reads will be included as well - MINIMUM_INSERT_SIZE and MAXIMUM_INSERT_SIZE will be ignored in CollectSequencingArtifactMetrics.  Default value: false. Possible values: {true, false} "
                ),
            ),
            ToolInput(
                tag="intervals",
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="Set of metrics programs to apply during the pass through the SAM file. This argument may be specified 0 or more times. Default value: [CollectAlignmentSummaryMetrics, CollectBaseDistributionByCycle, CollectInsertSizeMetrics, MeanQualityByCycle, QualityScoreDistribution]. Possible values: {CollectAlignmentSummaryMetrics, CollectInsertSizeMetrics, QualityScoreDistribution, MeanQualityByCycle, CollectBaseDistributionByCycle, CollectGcBiasMetrics, RnaSeqMetrics, CollectSequencingArtifactMetrics, CollectQualityYieldMetrics} "
                ),
            ),
            ToolInput(
                tag="quiet",
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="Gene annotations in refFlat form. Format described here: http://genome.ucsc.edu/goldenPath/gbdDescriptionsOld.html#RefFlat  Default value: null. "
                ),
            ),
            ToolInput(
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T14:09:55.911674"),
            dateUpdated=datetime.fromisoformat("2020-05-18T14:09:55.911675"),
            documentation="b'USAGE: CollectMultipleMetrics [arguments]\nCollect multiple classes of metrics.This 'meta-metrics' tool runs one or more of the metrics collection modules at the\nsame time to cut down on the time spent reading in data from input files. Available modules include\nCollectAlignmentSummaryMetrics, CollectInsertSizeMetrics, QualityScoreDistribution,  MeanQualityByCycle,\nCollectBaseDistributionByCycle, CollectGcBiasMetrics, RnaSeqMetrics, CollectSequencingArtifactMetrics and\nCollectQualityYieldMetrics. The tool produces outputs of '.pdf' and '.txt' files for each module, except for the\nCollectAlignmentSummaryMetrics module, which outputs only a '.txt' file. Output files are named by specifying a base\nname (without any file extensions).<br /><br /><p>Currently all programs are run with default options and fixed output\nextensions, but this may become more flexible in future. Specifying a reference sequence file is required.</p><p>Note:\nMetrics labeled as percentages are actually expressed as fractions!</p><h4>Usage example (all modules on by\ndefault):</h4><pre>java -jar picard.jar CollectMultipleMetrics \\<br />      I=input.bam \\<br />      O=multiple_metrics\n\\<br />      R=reference_sequence.fasta <br /></pre><h4>Usage example (two modules only):</h4>java -jar picard.jar\nCollectMultipleMetrics \\<br />      I=input.bam \\<br />      O=multiple_metrics \\<br />      R=reference_sequence.fasta\n\\<br />      PROGRAM=null \\<br />      PROGRAM=QualityScoreDistribution \\<br />      PROGRAM=MeanQualityByCycle\n</pre><hr />\nVersion:4.1.3.0\n",
        )

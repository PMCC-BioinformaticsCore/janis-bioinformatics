...
class GatkFuncotatorBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "Funcotator"

    def friendly_name(self) -> str:
        return "GATK4: Funcotator"

    def tool(self) -> str:
        return "Gatk4Funcotator"
    ...
    def inputs(self):
        return [
            ...
            ToolInput(
                tag="fivePrimeFlankSize",
                input_type=Int(optional=True),
                prefix="--five-prime-flank-size",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc=" Variants within this many bases of the 5' end of a transcript (and not overlapping any part of the transcript itself) will be annotated as being in the 5' flanking region of that transcript  Default value: 5000. "
                ),
            ),
            ToolInput(
                tag="gatkConfigFile",
            ...
            ToolInput(
                tag="removeFilteredVariants",
                input_type=Boolean(optional=True),
                prefix="--remove-filtered-variants",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc=" Ignore/drop variants that have been filtered in the input.  These variants will not appear in the output file.  Default value: false. Possible values: {true, false} "
                ),
            ),
            ToolInput(
            ...
            ToolInput(
                tag="threePrimeFlankSize",
                input_type=Int(optional=True),
                prefix="--three-prime-flank-size",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc=" Variants within this many bases of the 3' end of a transcript (and not overlapping any part of the transcript itself) will be annotated as being in the 3' flanking region of that transcript  Default value: 0. "
                ),
            ),
            ToolInput(
                tag="tmpDir",

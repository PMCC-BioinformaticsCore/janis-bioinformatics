...
class GatkTagGermlineEventsBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "TagGermlineEvents"

    def friendly_name(self) -> str:
        return "GATK4: TagGermlineEvents"

    def tool(self) -> str:
        return "Gatk4TagGermlineEvents"

    def inputs(self):
        return [
            ToolInput(
                tag="calledMatchedNormalSegFile",
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-O) Output TSV file identical to the tumor segment file, but with additional germline tag column (POSSIBLE_GERMLINE).  Required. "
                ),
            ),
            ToolInput(
                tag="reference",
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-S) Input tumor (called) segment file -- the output will be a copy of this file with the additional information.  Required. "
                ),
            ),
            ToolInput(
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="Amount of padding, in bases, to allow a breakpoint match. Default value: 1000."
                ),
            ),
            ToolInput(
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="Column header to use for the call (amplified, neutral, or deleted). Default value: CALL."
                ),
            ),
            ToolInput(
            ...
            ToolInput(
                tag="reciprocalThreshold",
                input_type=Boolean(optional=True),
                prefix="--reciprocal-threshold",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="Over this threshold will cause a tumor segment to be tagged.  Default value: 0.75. "
                ),
            ),
            ToolInput(
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T14:06:23.708918"),
            dateUpdated=datetime.fromisoformat("2020-05-18T14:06:23.708919"),
            documentation="**EXPERIMENTAL FEATURE - USE AT YOUR OWN RISK**\nUSAGE: TagGermlineEvents [arguments]\nA tool for tagging possible germline events in a tumor segment file.  The algorithm used is very simplistic.  Segments\ncalled as amplified or deleted in the normal are matched by breakpoints (+/- some padding) or reciprocal overlap.\nVersion:4.1.3.0\n",
        )

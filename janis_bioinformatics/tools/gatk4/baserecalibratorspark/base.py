...
class GatkBaseRecalibratorSparkBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "BaseRecalibratorSpark"

    def friendly_name(self) -> str:
        return "GATK4: BaseRecalibratorSpark"

    def tool(self) -> str:
        return "Gatk4BaseRecalibratorSpark"
    ...
    def inputs(self):
        return [
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-O) Path to save the final recalibration tables to. Required."
                ),
            ),
            ToolInput(
                tag="reference",
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-DF)  Read filters to be disabled before analysis  This argument may be specified 0 or more times. Default value: null. Possible Values: {MappedReadFilter, MappingQualityAvailableReadFilter, MappingQualityNotZeroReadFilter, NotDuplicateReadFilter, NotSecondaryAlignmentReadFilter, PassesVendorQualityCheckReadFilter, WellformedReadFilter}"
                ),
            ),
            ToolInput(
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T14:56:00.511274"),
            dateUpdated=datetime.fromisoformat("2020-05-18T14:56:00.511275"),
            documentation="**BETA FEATURE - WORK IN PROGRESS**\nUSAGE: BaseRecalibratorSpark [arguments]\nFirst pass of the Base Quality Score Recalibration (BQSR) on Spark. Generate a recalibration table based on various\nuser-specified covariates (such as read group, reported quality score, machine cycle, and nucleotide context).\nVersion:4.1.3.0\n",
        )

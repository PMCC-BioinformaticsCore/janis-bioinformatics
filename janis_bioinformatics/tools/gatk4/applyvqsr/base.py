...
class GatkApplyVQSRBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "ApplyVQSR"

    def friendly_name(self) -> str:
        return "GATK4: ApplyVQSR"

    def tool(self) -> str:
        return "Gatk4ApplyVQSR"
        ...
        return [
            ToolInput(
                ...
                doc=InputDocumentation(
                    doc="(-O) The output filtered and recalibrated VCF file in which each variant is annotated with its VQSLOD value  Required. "
                ),
            ),
            ToolInput(
                tag="recalFile",
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="Don't output filtered loci after applying the recalibration Default value: false. Possible values: {true, false} "
                ),
            ),
            ToolInput(
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="If specified, the recalibration will be applied to variants marked as filtered by the specified filter name in the input VCF file  This argument may be specified 0 or more times. Default value: null. "
                ),
            ),
            ToolInput(
                tag="inp",
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-mode) Recalibration mode to employ: 1.) SNP for recalibrating only SNPs (emitting indels untouched in the output VCF); 2.) INDEL for indels; and 3.) BOTH for recalibrating both SNPs and indels simultaneously.  Default value: SNP. Possible values: {SNP, INDEL, BOTH} "
                ),
            ),
            ToolInput(
                tag="quiet",
            ...
            ToolInput(
                tag="truthSensitivityFilterLevel",
                input_type=Double(optional=True),
                prefix="--truth-sensitivity-filter-level",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-ts-filter-level)  The truth sensitivity level at which to start filtering  Default value: null. "
                ),
            ),
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-AS)  If specified, the tool will attempt to apply a filter to each allele based on the input tranches and allele-specific .recal file.  Default value: false. Possible values: {true, false} "
                ),
            ),
            ToolInput(
                tag="useJdkDeflater",
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T15:05:38.743079"),
            dateUpdated=datetime.fromisoformat("2020-05-18T15:05:38.743080"),
            documentation="USAGE: ApplyVQSR [arguments]\nApply a score cutoff to filter variants based on a recalibration table\nVersion:4.1.3.0\n",
        )

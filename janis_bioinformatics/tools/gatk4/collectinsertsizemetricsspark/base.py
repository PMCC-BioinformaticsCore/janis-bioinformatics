...
class GatkCollectInsertSizeMetricsSparkBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "CollectInsertSizeMetricsSpark"

    def friendly_name(self) -> str:
        return "GATK4: CollectInsertSizeMetricsSpark"

    def tool(self) -> str:
        return "Gatk4CollectInsertSizeMetricsSpark"

    def inputs(self):
        return [
            ToolInput(
                tag="histogramPlotFile",
                input_type=String(optional=True),
                prefix="--histogram-plot-file",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-H)  File to write insert size histogram chart to.  Required. "
                ),
            ),
            ToolInput(
                tag="inp",
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-DF)  Read filters to be disabled before analysis  This argument may be specified 0 or more times. Default value: null. Possible Values: {FirstOfPairReadFilter, MappedReadFilter, MappingQualityReadFilter, NonZeroFragmentLengthReadFilter, NotDuplicateReadFilter, NotSecondaryAlignmentReadFilter, NotSupplementaryAlignmentReadFilter, PairedReadFilter, ProperlyPairedReadFilter, WellformedReadFilter}"
                ),
            ),
            ToolInput(
            ...
            ToolInput(
                tag="metricAccumulationLevel",
                input_type=Boolean(optional=True),
                prefix="--metric-accumulation-level",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-LEVEL)  The level(s) at which to accumulate metrics. Possible values are {ALL_READS, SAMPLE, LIBRARY, READ GROUP}.  This argument may be specified 0 or more times. Default value: [ALL_READS]. Possible values: {ALL_READS, SAMPLE, LIBRARY, READ_GROUP} "
                ),
            ),
            ToolInput(
                tag="minCategoryReadsPercentage",
                input_type=Float(optional=True),
                prefix="--min-category-reads-percentage",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-M)  When generating the histogram, discard any data categories (out of FR, TANDEM, RF) that have fewer than this percentage of overall reads (Range: 0 to 1).  Default value: 0.05. "
                ),
            ),
            ToolInput(
                tag="numReducers",
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="If enabled, an output .pdf plot will be created. Default value: false. Possible values: {true, false} "
                ),
            ),
            ToolInput(
                tag="programName",
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-W) Explicitly sets the histogram width, overriding automatic truncation of Histogram tail. Also, when calculating mean and standard deviation, only bins <= HISTOGRAM_WIDTH will be included.  Default value: null. "
                ),
            ),
            ToolInput(
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T14:09:43.242944"),
            dateUpdated=datetime.fromisoformat("2020-05-18T14:09:43.242945"),
            documentation="**BETA FEATURE - WORK IN PROGRESS**\nUSAGE: CollectInsertSizeMetricsSpark [arguments]\nCollects insert size distribution information in SAM/BAM/CRAM file(s). The tool leverages the Spark framework for faster\noperation.\nVersion:4.1.3.0\n",
        )

...
class GatkCountVariantsBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "CountVariants"

    def friendly_name(self) -> str:
        return "GATK4: CountVariants"

    def tool(self) -> str:
        return "Gatk4CountVariants"
    ...
    def inputs(self):
        return [
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-VS)  Validation stringency for all SAM/BAM/CRAM/SRA files read by this program.  The default stringency value SILENT can improve performance when processing a BAM file Tool returned: 0 in which variable-length data (read, qualities, tags) do not otherwise need to be decoded.  Default value: SILENT. Possible values: {STRICT, LENIENT, SILENT} "
                ),
            ),
            ToolInput(
                tag="reference",
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T15:04:04.187396"),
            dateUpdated=datetime.fromisoformat("2020-05-18T15:04:04.187398"),
            documentation="USAGE: CountVariants [arguments]\nThis tool counts the variant records in a VCF file, regardless of filter status. Because it counts the number of rows in\nthe VCF, it does not necessarily reflect the number of variant alleles. The count is returned at the end of the standard\nout.\nVersion:4.1.3.0\n",
        )

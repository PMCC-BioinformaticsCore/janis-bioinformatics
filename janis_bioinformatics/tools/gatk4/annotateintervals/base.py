...
class GatkAnnotateIntervalsBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "AnnotateIntervals"

    def friendly_name(self) -> str:
        return "GATK4: AnnotateIntervals"

    def tool(self) -> str:
        return "Gatk4AnnotateIntervals"
    ...
    def inputs(self):
        return [
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-O) Output file for annotated intervals. Required."
                ),
            ),
            ToolInput(
                tag="reference",
            ...
            ToolInput(
                tag="segmentalDuplicationTrack",
                input_type=Boolean(optional=True),
                prefix="--segmental-duplication-track",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc=" Path to segmental-duplication track in .bed or .bed.gz format.  Overlapping intervals must be merged.  Default value: null. "
                ),
            ),
            ToolInput(
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T14:04:53.947913"),
            dateUpdated=datetime.fromisoformat("2020-05-18T14:04:53.947914"),
            documentation="USAGE: AnnotateIntervals [arguments]\nAnnotates intervals with GC content, mappability, and segmental-duplication content\nVersion:4.1.3.0\n",
        )

...
class GatkCollectMultipleMetricsSparkBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "CollectMultipleMetricsSpark"

    def friendly_name(self) -> str:
        return "GATK4: CollectMultipleMetricsSpark"

    def tool(self) -> str:
        return "Gatk4CollectMultipleMetricsSpark"
    ...
    def inputs(self):
        return [
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="List of metrics collectors to apply during the pass through the SAM file. If no collectors are specified than all collectors will be run  This argument may be specified 0 or more times. Default value: null. Possible values: {CollectInsertSizeMetrics, CollectQualityYieldMetrics} "
                ),
            ),
            ToolInput(
                tag="conf",
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T14:10:02.805076"),
            dateUpdated=datetime.fromisoformat("2020-05-18T14:10:02.805077"),
            documentation="**BETA FEATURE - WORK IN PROGRESS**\nUSAGE: CollectMultipleMetricsSpark [arguments]\nInstantiates and executes multiple metrics collection tasks for a given SAM/BAM/CRAM file. The tool leverages the Spark\nframework for faster operation. Importantly, the aligned reads are loaded into a Spark RDD only once, and the same RDD\nis used for each metric collection tasks to cut down on I/O time. Currently all programs are run  with default options\nand fixed output extensions.\nVersion:4.1.3.0\n",
        )

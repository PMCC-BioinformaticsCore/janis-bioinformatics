...
class GatkRevertOriginalBaseQualitiesAndAddMateCigarBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "RevertOriginalBaseQualitiesAndAddMateCigar"

    def friendly_name(self) -> str:
        return "GATK4: RevertOriginalBaseQualitiesAndAddMateCigar"

    def tool(self) -> str:
        return "Gatk4RevertOriginalBaseQualitiesAndAddMateCigar"
    ...
    def inputs(self):
        return [
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-O) The output SAM/BAM file to create. Required."
                ),
            ),
            ToolInput(
                tag="arguments_file",
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="Whether to create an MD5 digest for any BAM or FASTQ files created. Default value: true. Possible values: {true, false} "
                ),
            ),
            ToolInput(
            ...
            ToolInput(
                tag="max_records_to_examine",
                input_type=Int(optional=True),
                prefix="--MAX_RECORDS_TO_EXAMINE",
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc=" The maximum number of records to examine to determine if we can exit early and not output, given that there are a no original base qualities (if we are to restore) and mate cigars exist. Set to 0 to never skip the file.  Default value: 10000. "
                ),
            ),
            ToolInput(
                tag="quiet",
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-SO) The sort order to create the reverted output file with.By default, the sort order will be the same as the input.  Default value: null. Possible values: {unsorted, queryname, coordinate, duplicate, unknown} "
                ),
            ),
            ToolInput(
                tag="tmp_dir",
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T14:58:43.111160"),
            dateUpdated=datetime.fromisoformat("2020-05-18T14:58:43.111161"),
            documentation="b'USAGE: RevertOriginalBaseQualitiesAndAddMateCigar [arguments]\nReverts the original base qualities and adds the mate cigar tag to read-group BAMs.\nVersion:4.1.3.0\n",
        )

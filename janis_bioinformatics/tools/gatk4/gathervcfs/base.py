...
from janis import ToolInput, Filename, ToolOutput, Array, String, InputSelector, File, Boolean, Int, CaptureType
...
class Gatk4GatherVcfsBase(Gatk4ToolBase, ABC):
    ...
    def inputs(self):
        return [
            ToolInput("vcfs", Array(Vcf()), prefix="--INPUT", doc="[default: []] (-I) Input VCF file(s)."),
            ToolInput("outputFilename", Filename(extension=".vcf", suffix=".gathered"), prefix="--OUTPUT",
...
""".strip()
    ...
    additional_args = [
        ToolInput("argumentsFile", Array(File(), optional=True), prefix="--arguments_file",
                  doc="[default: []] read one or more arguments files and add them to the command line"),
        ToolInput("compressionLevel", Int(optional=True), prefix="--COMPRESSION_LEVEL",
                  doc="[default: 5] Compression level for all compressed files created (e.g. BAM and VCF)."),
        ToolInput("createIndex", Boolean(optional=True), prefix="--CREATE_INDEX",
                  doc="[default: TRUE] Whether to create a BAM index when writing a coordinate-sorted BAM file."),
        ToolInput("createMd5File", Boolean(optional=True), prefix="--CREATE_MD5_FILE",
                  doc="[default: FALSE] Whether to create an MD5 digest for any BAM or FASTQ files created."),
        ToolInput("ga4ghClientSecrets", File(optional=True), prefix="--GA4GH_CLIENT_SECRETS",
                  doc="[default: client_secrets.json] Google Genomics API client_secrets.json file path."),
        ToolInput("maxRecordsInRam", Int(optional=True), prefix="--MAX_RECORDS_IN_RAM",
        ...
        ToolInput("referenceSequence", File(optional=True), prefix="--REFERENCE_SEQUENCE",
                  doc="[default: null] Reference sequence file."),
        ToolInput("tmpDir", String(optional=True), default="/tmp", prefix="--TMP_DIR",
        ...
        ToolInput("verbosity", Boolean(optional=True), prefix="--VERBOSITY",

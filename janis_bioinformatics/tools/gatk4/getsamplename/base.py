...
class GatkGetSampleNameBase(Gatk4ToolBase, ABC):
    @classmethod
    def gatk_command(cls):
        return "GetSampleName"

    def friendly_name(self) -> str:
        return "GATK4: GetSampleName"

    def tool(self) -> str:
        return "Gatk4GetSampleName"
    ...
    def inputs(self):
        return [
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-O) Output file with only the sample name in it. Required."
                ),
            ),
            ToolInput(
            ...
            ToolInput(
                ...
                separate_value_from_prefix=True,
                doc=InputDocumentation(
                    doc="(-encode)  Apply URL encoding to convert spaces and other special characters in sample name.  Default value: false. Possible values: {true, false} "
                ),
            ),
            ToolInput(
                tag="verbosity",
    ...
    def metadata(self):
        return ToolMetadata(
            contributors=[],
            dateCreated=datetime.fromisoformat("2020-05-18T14:51:32.167941"),
            dateUpdated=datetime.fromisoformat("2020-05-18T14:51:32.167942"),
            documentation="**BETA FEATURE - WORK IN PROGRESS**\nUSAGE: GetSampleName [arguments]\nEmit a single sample name from the bam header into an output file. The sample name is that in the read group (RG) sample\n(SM) field\nVersion:4.1.3.0\n",
        )

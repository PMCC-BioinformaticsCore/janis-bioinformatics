...
class CompressVCF(BioinformaticsWorkflow):
    def __init__(self):
        super(CompressVCF, self).__init__(
            "compressVCF", friendly_name="Compress VCF File"
        )

        inp = Input(
            "vcf",
            Vcf(),
            "/Users/franklinmichael/reference/hg38/snps_dbsnp/Homo_sapiens_assembly38.dbsnp138.compressable.vcf",
        )

        s1_bgzip = Step("s1_bgzip", BGZip_1_2_1())
        s2_tabix = Step("s2_tabix", Tabix_1_2_1())

        self.add_edges([(inp, s1_bgzip.file), (s1_bgzip.out, s2_tabix.file)])

        self.add_edges([(s2_tabix.out, Output("ret"))])

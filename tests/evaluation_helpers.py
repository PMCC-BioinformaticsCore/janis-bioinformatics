...
def evaluate(tool) -> Dict[str, str]:
    ...
    if evaluation['unit_tests_exists'] is True:
        result = run_tool_unit_tests(tool)
        count_failed = len(result.failures) + len(result.errors)

        if count_failed > 0:
            evaluation['unit_tests_run'] = f"{count_failed} unit test failed"
        else:
            evaluation['unit_tests_run'] = True
...
def evaluate_translation(tool: jc.Tool):
    ...
    tool.translate(SupportedTranslation.CWL, to_console=False, to_disk=True, export_path=cwl_file_path)
    tool.translate(SupportedTranslation.WDL, to_console=False, to_disk=True, export_path=wdl_file_path)
